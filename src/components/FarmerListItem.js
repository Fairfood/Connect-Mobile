...
const FarmerListItem = ({ item, displayClose, onClose, onPress, displayUnSync, avatarBgColor = '#607d8b' }) => {
  return (
    <View style={[styles.container, { backgroundColor: displayClose ? '#DDF3FF' : null }]}>
      <TouchableOpacity
      ...
      >
        ...
        {displayUnSync && (item.server_id === '' || item.is_modified || item.is_card_modified) && (
          <TouchableOpacity
            onPress={onClose}
            style={[styles.person, { backgroundColor: null, marginRight: 0, width: '10%' }]}
        ...
        {displayClose && (
          <TouchableOpacity
            onPress={onClose}
            style={[styles.person, { backgroundColor: null, marginRight: 0, width: '15%' }]}
            hitSlop={consts.HIT_SLOP_TWENTY}
          >
