...
const FarmersHomeScreen = ({ navigation, FARMERS }) => {
  ...
  const [sortValue, setSortValue] = useState(consts.SortMenus[0].key);
  ...
  const sortFarmers = async (farmers, sortKey) => {
    const key = sortKey || sortValue || consts.SortMenus[0].key;
  ...
  const onRefresh = async () => {
    setSearchText('');
    const sortedFarmers = await sortFarmers(FARMERS, consts.SortMenus[0].key);
    setFarmersList(sortedFarmers);
    setSortValue(consts.SortMenus[0].key);
  };
    ...
    <FarmerListItem
      ...
      avatarBgColor={
        consts.AVATAR_AS_LETTERS
          ? consts.AvatarBgColors[index % consts.AvatarBgColors.length]
          : null
      }
      displayUnSync
    />
