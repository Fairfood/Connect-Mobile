...
import {
  findAllPremiumsByProductAndPremium,
  saveProductPremium,
...
export const getProductsList = async () => {
  ...
  const config = {
    url: `${api.API_URL}${api.API_VERSION}/projects/project/${loggedInUser.project_id}/product/?status=all`,
    headers,
  };
  ...
  if (response.success) {
    ...
    products.map(async (product) => {
      ...
      if (isExisting.length === 0) {
        ...
        await updateProductActiveStatus(isExisting[0].id, product.is_active);
      }

      // saving product_premiums
      if (product?.premiums && product.premiums.length > 0) {
        const { premiums } = product;
        premiums.map(async (premium) => {
          const isExistingPremium = await findAllPremiumsByProductAndPremium(
            product.id,
            premium,
          );
          if (isExistingPremium.length === 0) {
            saveProductPremium(product.id, premium);
          }
        });
      }
    });
...
const getTransactionsList = async (url) => {
  ...
  const defaultUrl = `${api.API_URL}${api.API_VERSION}/projects/transaction/list/?limit=50&offset=0${updatedAfter}${projectIdParam}`;
      ...
      if (isExisting.length === 0) {
        ...
        if (transaction.category === consts.APP_TRANS_TYPE_INCOMING) {
          const batch = {
            ...
            current_quantity:
              transaction.destination_batches[0].current_quantity,
